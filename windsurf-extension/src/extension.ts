...
import { WindsurfBridge } from './websocket-client';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const windsurfBridge = new WindsurfBridge();
    ...
    // Update status bar
    const updateStatusBar = (connected: boolean, message?: string) => {
    ...
    const handleDoneDepMessage = async (message: any) => {
        const config = vscode.workspace.getConfiguration('donedep');
        ...
        switch (message.action) {
            ...
            case 'git.createPR':
                const prUrl = await gitOps.createPullRequest(
            ...
            case 'error.notify':
                if (config.get('notifyOnErrors', true)) {
                    const action = await vscode.window.showErrorMessage(
                        `DoneDep Error: ${message.payload.message}`,
                        'View Details',
                        'Fix Automatically'
                    );
                ...
                if (config.get('notifyOnErrors', true)) {
                    ...
                    if (status === 'success') {
                        vscode.window.showInformationMessage(
                            `Deployment successful! URL: ${message.payload.url}`
                        );
                    } else if (status === 'failed') {
                        vscode.window.showErrorMessage(
                            `Deployment failed: ${message.payload.error}`
                        );
                    }
                }
                break;
    ...
    const autoConnect = vscode.workspace.getConfiguration('donedep').get<boolean>('autoConnect', true);
    if (autoConnect) {
